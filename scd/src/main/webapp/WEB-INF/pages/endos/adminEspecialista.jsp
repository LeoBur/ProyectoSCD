...
<head>
	...
	<script type="text/javascript">
		function CancelFormButton(button) {
			onsubmit: false;
	  	window.location.href = "${ctx}/endos/endo";
	  	};
	</script>
    ...
    $(function(){
      $("select#provincia").change(function(){
        $.getJSON("/getLocalidades1?provincia="+$(this).val(), function(j){
          var options = '';
...
<div class="col-sm-7">
    ...
    <form:form commandName="especialistaForm" method="post" action="adminEspecialista" id="formulario" autocomplete="off"
					  ...
					  <span class="required">*</span>
                        ...
                        <c:choose>
                            <c:when test="${especialistaForm.sexo == null || especialistaForm.sexo == 'M'}">
                                <input type="radio" name="sexo" value="M" checked="checked" tabindex="7"/>Masculino  &nbsp; &nbsp; &nbsp;
                                <input type="radio" name="sexo" value="F" tabindex="7"/>Femenino
                            </c:when>
                            <c:when test="${especialistaForm.sexo != null && especialistaForm.sexo == 'F'}">
                                <input type="radio" name="sexo" value="M" tabindex="7"/>Masculino &nbsp; &nbsp; &nbsp;
                                <input type="radio" name="sexo" value="F" checked="checked" tabindex="7"/>Femenino
                            </c:when>
                        </c:choose>
						</div>
		...
		<div class="row">
					...
					<spring:bind path="especialistaForm.provincia">
						<div class="col-sm-6 form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
						  ...
						  <div cssClass="form-control">
							<form:select id="provincia" name="provincia" class="form-control" path="provincia" value="${status.value}" tabindex="10">
						...
						<div class="col-sm-6 form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
							...
							<div cssClass="form-control">
								<form:select id="localidad" name="localidad" class="form-control"
								 path="localidad" tabindex="11" value="${status.value}">
				...
				<spring:bind path="especialistaForm.piso">
					<div class="col-sm-6 form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
						...
						<div cssClass="form-control">
							...
							placeholder="<fmt:message key="user.address.piso"/>" value="${status.value}" autocomplete="off" tabindex="15"/>
         ...
         <div class="row">
                <div class="form-group">
                 	<appfuse:label styleClass="control-label" key="user.endocrinologist.tipoEspecialista"/>
                 	<form:select cssClass="form-control" path="especialistaForm.tipo_esp" value="${status.value}" tabindex="16">
                 		<form:option value="NUTRICIONISTA" label="NUTRICIONISTA"/>
                 		<form:option value="PERSONAL_TRAINER" label="PERSONAL_TRAINER" selected="selected"/>
                 	</form:select>
                 </div>
       	...
       	<div class="form-group">
			...
			<button type="submit" class="btn btn-default" name="cancel" onclick="bCancel=true" tabindex="21">
...
<c:set var="scripts" scope="request">
	...
	<script type="text/javascript">
		 $('button[name="cancel"]').click(function(e){
				e.preventDefault();
				window.location.href = "${ctx}/endos/adminEspecialista";
			});
	</script>
	<script type="text/javascript">
	...
	<script type="text/javascript">
    		 $('button[name="search"]').click(function(e){
    				...
    				window.location.href = "${ctx}/endos/adminEspecialista?search=search&dni="+dni;
    			});
    </script>
